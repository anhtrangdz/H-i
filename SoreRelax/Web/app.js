...
async function enterApp(){
  ...
  $('#profileName').textContent=app.data.settings.displayName||'Prix'; $('#profileAvatar').textContent=(app.data.settings.displayName||'P').trim()[0]?.toUpperCase()||'P';
  ...
  applySettings(); render(location.hash.slice(1)||'home'); resetActivity();
}
...
function applySettings(){
  ...
  $('#privacyButton').textContent=app.data?.settings?.privacy?'◌':'◉';
  $('#moodTheme span:last-child').textContent=app.data?.settings?.evening?'Buổi tối':'Thư giãn';
}
...
function monthControls(){ return `<button class="button-soft" data-prev-month>‹</button><button class="button-soft" data-month-label>${esc(monthLabel())}</button><button class="button-soft" data-next-month>›</button>`; }
...
function moneyPage(){
  ...
  return `${pageHead('TÀI CHÍNH',`Tài chính · ${esc(monthLabel())}`,'Bạn nhập tổng tiền có thể dùng trong tháng. Mỗi khoản chi sẽ tự trừ khỏi số còn lại; chuyển tiền giữa các tài khoản không làm giảm số này.',`${monthControls()}<button class="button-soft" data-set-month-total>Chỉnh tổng tiền tháng</button><button class="button-primary" data-open-quick>＋ Thêm giao dịch</button>`)}
...
function budgetsPage(){
  ...
  return `${pageHead('NGÂN SÁCH',`Ngân sách · ${esc(monthLabel())}`,'Đặt giới hạn theo từng nhóm để dễ theo dõi. Chi tiêu thực tế tự cập nhật vào đúng danh mục.',`${monthControls()}<button class="button-primary" data-add-budget>＋ Thêm ngân sách</button>`)}
...
function dailyPage(){
  ...
  return `${pageHead('NHẬT KÝ HẰNG NGÀY','Nhật ký hằng ngày','Viết về một ngày, chọn tâm trạng và thêm ảnh. Không giới hạn số lượng ảnh trong một ngày; ảnh được mã hóa và lưu cục bộ trên thiết bị.',`${monthControls()}<button class="button-soft" data-daily-today>Hôm nay</button>`)}
...
function render(route=app.route,scroll=true){ if(!pages[route])route='home'; app.route=route; pageWrap.dataset.route=route; pageWrap.innerHTML=pages[route](); bindPageEvents(); $$('[data-route]').forEach(el=>el.classList.toggle('active',el.dataset.route===route)); $$('.mobile-nav-item').forEach(el=>el.classList.toggle('active',el.dataset.route===route)); if(scroll)window.scrollTo({top:0,behavior:'smooth'}); history.replaceState(null,'',`#${route}`); }
function navigate(route){ $('.sidebar')?.classList.remove('open'); render(route); }

function bindGlobal(){
  document.addEventListener('click',e=>{const r=e.target.closest('[data-route]');if(r){e.preventDefault();navigate(r.dataset.route);}});
  $('#sidebarCollapse').addEventListener('click',()=>appShell.classList.toggle('collapsed'));
  $('#mobileMenu').addEventListener('click',()=>$('.sidebar').classList.toggle('open'));
  $('#quickAdd').addEventListener('click',()=>openQuick('expense')); $('#mobileQuickAdd').addEventListener('click',()=>openQuick('expense'));
  $('#privacyButton').addEventListener('click',()=>saveSetting({privacy:!app.data.settings.privacy}));
  $('#moodTheme').addEventListener('click',()=>saveSetting({evening:!app.data.settings.evening}));
  $('#lockButton').addEventListener('click',logout);
  $('#searchTrigger').addEventListener('click',openCommand);
  document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();openCommand();}if(e.key==='Escape'){closeQuick();closeForm();closeCommand();}});
  $$('[data-close="quick"]').forEach(b=>b.addEventListener('click',closeQuick)); $$('[data-close="form"]').forEach(b=>b.addEventListener('click',closeForm));
  $('.quick-tabs').addEventListener('click',e=>{const b=e.target.closest('[data-type]');if(b)setQuickType(b.dataset.type);});
  $('#quickForm').addEventListener('submit',submitQuick);
  $('#commandInput').addEventListener('input',renderCommandResults);
  ['pointerdown','keydown','touchstart','mousemove'].forEach(ev=>document.addEventListener(ev,resetActivity,{passive:true}));
...
function openProfileForm(){openForm('Tên hiển thị','TÀI KHOẢN',`<form id="profileForm" class="form-stack"><label class="field"><span>Tên hiển thị</span><input id="profileValue" maxlength="60" value="${esc(app.data.settings.displayName)}"></label><div class="modal-actions"><button type="button" class="button-ghost" data-cancel>Hủy</button><button class="button-primary">Lưu</button></div></form>`);$('[data-cancel]',formBody).onclick=closeForm;$('#profileForm').onsubmit=async e=>{e.preventDefault();try{await saveSetting({displayName:$('#profileValue').value});$('#profileName').textContent=app.data.settings.displayName;closeForm();toast('Đã đổi tên hiển thị.');}catch(err){onError(err);}};}
